...
| Where should a first adopter start? | README "Choose An API Route", `docs/recipes/quickstart-adoption.md`, website "Choose your path" section | TUI/CLI library authors can start with stable core APIs, then opt into incubating large-text, rich, and source-first workflows as needed. |
...
| What validates portable conformance cases? | `fixtures/conformance/*.json`, `fixtures/conformance/README.md`, `bun run conformance-kit-check`, `bun run scripts/tui-conformance-kit-generate.ts --check` | The repo-only kit pins width, wrap, and offset behavior for review without expanding the npm tarball. |
...
```text
See clean evidence report competitive-tui-20260615-05a8d54-clean-ad380eea for raw samples, provenance metadata, workload ids, and semantic caveats.
```
...
- The quickstart adoption route stays aligned across README, recipes, and website copy.
- Claims separate release gates from optional comparison reports.
- Conformance-kit claims stay repo-only and do not imply npm tarball contents.
