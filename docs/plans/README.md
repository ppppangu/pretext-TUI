<!-- 补建说明：该目录为后续补建，用于集中保存 pretext-TUI 的分阶段实施计划与校验记录，方便后续 swarm 执行与追踪；当前进度：已包含终端布局迁移计划、post-publishability master plan 与 Phase 8 append-only chunked storage plan。 -->
# Plan Index

This directory stores execution plans for `pretext-TUI`.

Current purpose:
- hold detailed implementation plans with exact files, commands, and validation steps
- act as the canonical handoff surface for later execution sessions
...
- `2026-06-10-prefix-eviction-design-rfc.md` is the R3 item 5 design record (RFC only) for destructive prefix eviction: coordinate-domain candidates, blast radius, boundary constraints, and approval prerequisites before any implementation
