...
expectIncludes(pretty.stdout, '52 cols -> 32 rows', 'pretty primary row count')
expectIncludes(pretty.stdout, '[2] resize reflow', 'pretty resize section')
expectIncludes(pretty.stdout, '36 cols -> 40 rows', 'pretty narrow resize row count')
...
expectEqual(baseLayout['rows'], 32, 'json base rows')
...
expectEqual(expectRecord(reflow[0], 'json reflow[0]')['rows'], 40, 'json reflow narrow rows')
expectEqual(expectRecord(reflow[1], 'json reflow[1]')['columns'], 52, 'json reflow base columns')
expectEqual(expectRecord(reflow[1], 'json reflow[1]')['rows'], 32, 'json reflow base rows')
...
expectEqual(expectRecord(lines[0], 'json first viewport line')['text'], '$ pretext-tui demo --profile ', 'json first line text')
expectEqual(expectRecord(lines[4], 'json wrapped URL line')['width'], 52, 'json wrapped URL line width')
