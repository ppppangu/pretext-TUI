...
async function smokeJavaScriptEsm(tarballPath: string): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      "const tailPrepared = root.prepareTerminal('t0\\nt1\\nt2\\nt3\\nt4\\nt5', { whiteSpace: 'pre-wrap' })",
      "const tailBundle = root.createTerminalLayoutBundle(tailPrepared, { columns: 8, anchorInterval: 2, pageSize: 4, maxPages: 2 })",
      "const tailTotal = root.measureTerminalLayoutBundleRows(tailPrepared, tailBundle)",
      "const tailPage = root.getTerminalLayoutBundleTailPage(tailPrepared, tailBundle, { rowCount: 4 })",
      "if (!Object.isFrozen(tailPage) || !Object.isFrozen(tailPage.lines)) throw new Error('bundle tail page must be frozen')",
      "if (tailPage.startRow !== Math.max(0, tailTotal - 4)) throw new Error('bad bundle tail startRow')",
      "if (tailPage.rowCount !== Math.min(4, tailTotal)) throw new Error('bad bundle tail rowCount')",
      "const tailLineIndex = root.createTerminalLineIndex(tailPrepared, { columns: 8, anchorInterval: 2 })",
      "const tailRanges = root.getTerminalLineIndexTailRanges(tailPrepared, tailLineIndex, { rowCount: 4 })",
      "if (tailRanges.length > 4) throw new Error('line-index tail exceeded rowCount')",
      "try { root.getTerminalLayoutBundleTailPage(tailPrepared, tailBundle, { rowCount: 0 }); throw new Error('bad bundle tail rowCount 0 accepted') } catch (error) { if (error.message === 'bad bundle tail rowCount 0 accepted') throw error; if (!error.message.includes('rowCount')) throw error }",
      "try { root.getTerminalLineIndexTailRanges(tailPrepared, tailLineIndex, { rowCount: 0 }); throw new Error('bad line-index tail rowCount 0 accepted') } catch (error) { if (error.message === 'bad line-index tail rowCount 0 accepted') throw error; if (!error.message.includes('rowCount')) throw error }",
      "try { root.getTerminalLayoutBundleTailPage(tailPrepared, Object.freeze({ kind: 'terminal-layout-bundle@1' }), { rowCount: 4 }); throw new Error('bad forged bundle tail accepted') } catch (error) { if (error.message === 'bad forged bundle tail accepted') throw error; if (!error.message.includes('Invalid terminal layout bundle handle')) throw error }",
      ...
      "const appendBaseRanges = [{ id: 'block', kind: 'generic', sourceStart: 0, sourceEnd: 3, tags: ['a'], data: { payloadId: 'p1' } }, { id: 'point', kind: 'generic-marker', sourceStart: 1, sourceEnd: 1 }]",
      "const appendNextRanges = [{ id: 'after', kind: 'generic', sourceStart: 2, sourceEnd: 5 }, { id: 'wide', kind: 'generic', sourceStart: 0, sourceEnd: 6 }]",
      "const appendedRangeIndex = root.appendTerminalRanges(root.createTerminalRangeIndex(appendBaseRanges), appendNextRanges)",
      "assertOpaqueHandle('appended range index', appendedRangeIndex)",
      "const oneShotRangeIndex = root.createTerminalRangeIndex([...appendBaseRanges, ...appendNextRanges])",
      "for (let offset = 0; offset <= 7; offset++) { if (root.getTerminalRangesAtSourceOffset(appendedRangeIndex, offset).map(range => range.id).join(',') !== root.getTerminalRangesAtSourceOffset(oneShotRangeIndex, offset).map(range => range.id).join(',')) throw new Error('append point parity mismatch at ' + offset) }",
      "for (let start = 0; start <= 6; start++) { for (let end = start; end <= 7; end++) { if (root.getTerminalRangesForSourceRange(appendedRangeIndex, { sourceStart: start, sourceEnd: end }).map(range => range.id).join(',') !== root.getTerminalRangesForSourceRange(oneShotRangeIndex, { sourceStart: start, sourceEnd: end }).map(range => range.id).join(',')) throw new Error('append range parity mismatch at ' + start + ',' + end) } }",
      "try { root.appendTerminalRanges(root.createTerminalRangeIndex(appendBaseRanges), [{ id: 'block', kind: 'generic', sourceStart: 4, sourceEnd: 5 }]); throw new Error('bad duplicate append id accepted') } catch (error) { if (error.message === 'bad duplicate append id accepted') throw error; if (!error.message.includes('id must be unique')) throw error }",
      "const emptyAppendedRangeIndex = root.appendTerminalRanges(appendedRangeIndex, [])",
      "assertOpaqueHandle('empty appended range index', emptyAppendedRangeIndex)",
      "if (root.getTerminalRangesAtSourceOffset(emptyAppendedRangeIndex, 1).map(range => range.id).join(',') !== root.getTerminalRangesAtSourceOffset(appendedRangeIndex, 1).map(range => range.id).join(',')) throw new Error('empty append changed query results')",
      ...
      "const uncappedStats = root.getTerminalSearchSessionStats(search)",
      "if (!Object.isFrozen(uncappedStats) || uncappedStats.kind !== 'terminal-search-session-stats@1' || uncappedStats.matchLimit !== null || uncappedStats.truncated !== false || uncappedStats.storedMatchCount !== 1) throw new Error('bad uncapped search stats')",
      "const cappedSearchPrepared = root.prepareTerminal('ababab', { whiteSpace: 'pre-wrap' })",
      "const cappedSearch = root.createTerminalSearchSession(cappedSearchPrepared, 'ab', { matchLimit: 2 })",
      "if (root.getTerminalSearchSessionMatchCount(cappedSearch) !== 2) throw new Error('bad capped search count')",
      "const cappedStats = root.getTerminalSearchSessionStats(cappedSearch)",
      "if (cappedStats.matchLimit !== 2 || cappedStats.storedMatchCount !== 2 || cappedStats.truncated !== true) throw new Error('bad capped search stats')",
      "try { root.createTerminalSearchSession(cappedSearchPrepared, 'ab', { matchLimit: 0 }); throw new Error('bad matchLimit accepted') } catch (error) { if (error.message === 'bad matchLimit accepted') throw error; if (!error.message.includes('matchLimit')) throw error }",
      ...
      "try { root.getTerminalSearchSessionStats(Object.freeze({ kind: 'terminal-search-session@1' })); throw new Error('bad forged search stats accepted') } catch (error) { if (error.message === 'bad forged search stats accepted') throw error }",
      ...
      "try { rich.prepareTerminalRichInline('x', { profile: 'unsupported-profile' }); throw new Error('bad profile accepted') } catch (error) { if (error.message === 'bad profile accepted') throw error }",
...
async function smokeTypeScript(tarballPath: string): Promise<void> {
  ...
  await writeFile(
    ...
    [
      `import { appendTerminalCellFlow, createTerminalLayoutBundle, createTerminalLineIndex, createTerminalPageCache, createTerminalRangeIndex, createTerminalSourceOffsetIndex, getTerminalCellFlowGeneration, getTerminalCellFlowPrepared, getTerminalLayoutBundlePage, getTerminalLinePage, getTerminalLineRangeAtRow, getTerminalRangesForSourceRange, invalidateTerminalLayoutBundle, materializeTerminalLinePage, prepareTerminal, prepareTerminalCellFlow, projectTerminalCoordinate, projectTerminalSourceOffset, projectTerminalSourceRange, type MaterializedTerminalLine, type TerminalProjectionIndexes, type TerminalRange, type TerminalSourceRangeProjection } from '${packageName}'`,
      ...
      'type AgentTranscriptRecord = { id: string; kind: "request" | "response" | "observation" | "status"; text: string }',
      'type AgentTranscriptSnapshot = { rows: readonly MaterializedTerminalRichLine[]; visibleRecordIds: readonly string[] }',
      '',
      ...
      '}',
      '',
      'function buildAgentTranscriptSource(records: readonly AgentTranscriptRecord[]): { source: string; ranges: readonly TerminalRange[] } {',
      "  let source = ''",
      '  const ranges: TerminalRange[] = []',
      '  for (const record of records) {',
      "    if (source.length > 0) source += '\\n\\n'",
      '    const sourceStart = source.length',
      '    source += `${record.kind.toUpperCase()}\\n${record.text}`',
      '    ranges.push({ id: record.id, kind: `agent-transcript:${record.kind}`, sourceStart, sourceEnd: source.length, tags: [record.kind], data: { recordId: record.id } })',
      '  }',
      '  return { source, ranges }',
      '}',
      '',
      ...
      '}',
      '',
      'export function recipeGenericAgentTranscript(records: readonly AgentTranscriptRecord[], columns: number): AgentTranscriptSnapshot {',
      '  const { source, ranges } = buildAgentTranscriptSource(records)',
      "  const rich = prepareTerminalRichInline(source, { whiteSpace: 'pre-wrap', profile: 'transcript', unsupportedControlMode: 'sanitize', rawRetention: 'fingerprint' })",
      '  const rangeIndex = createTerminalRangeIndex(ranges)',
      '  const bundle = createTerminalLayoutBundle(rich.prepared, { columns, anchorInterval: 64, pageSize: 64, maxPages: 4 })',
      '  const page = getTerminalLayoutBundlePage(rich.prepared, bundle, { startRow: 0, rowCount: 12 })',
      '  const rows = page.lines.map(line => materializeTerminalRichLineRange(rich, line))',
      '  const visibleRecordIds = [...new Set(page.lines.flatMap(line => getTerminalRangesForSourceRange(rangeIndex, { sourceStart: line.sourceStart, sourceEnd: line.sourceEnd }).map(range => range.id)))]',
      '  return { rows, visibleRecordIds }',
      '}',
      '',
      'export function recipeGenericAgentTranscriptAppend(seed: string, appendedText: string, columns: number): number {',
      "  const flow = prepareTerminalCellFlow(seed, { whiteSpace: 'pre-wrap' })",
      '  const bundle = createTerminalLayoutBundle(getTerminalCellFlowPrepared(flow), { columns, generation: getTerminalCellFlowGeneration(flow), pageSize: 64, maxPages: 4 })',
      '  const appended = appendTerminalCellFlow(flow, appendedText)',
      '  return invalidateTerminalLayoutBundle(getTerminalCellFlowPrepared(appended.flow), bundle, appended.invalidation).generation',
      '}',
      '',
      ...
      "const agentTranscriptSnapshot = recipeGenericAgentTranscript([{ id: 'u1', kind: 'request', text: 'hello' }, { id: 'a1', kind: 'response', text: '\\x1b[32mok\\x1b[0m' }], 12)",
      "if (agentTranscriptSnapshot.visibleRecordIds.join(',') !== 'u1,a1') throw new Error('recipe generic transcript should return visible record ids')",
      "if (!agentTranscriptSnapshot.rows.some(row => row.fragments.some(fragment => fragment.text.includes('ok') && fragment.style?.fg === 'ansi:32'))) throw new Error('recipe generic transcript should preserve rich fragments')",
      "if (recipeGenericAgentTranscriptAppend('REQUEST\\nhello', '\\n\\nRESPONSE\\nok', 12) !== 1) throw new Error('recipe generic transcript append should advance generation')",
  ...
  await writeFile(
    ...
    [
      ...
      'const tailRequest: TerminalLineIndexTailRequest = { rowCount: 2 }',
      'tailRequest satisfies TerminalLineIndexTailRequest',
      'measureTerminalLayoutBundleRows(prepared, bundle) satisfies number',
      'getTerminalLayoutBundleTailPage(prepared, bundle, tailRequest) satisfies TerminalLinePage',
      'getTerminalLineIndexTailRanges(prepared, lineIndex, tailRequest) satisfies readonly TerminalLineRange[]',
      ...
      "appendTerminalRanges(rangeIndex, [{ id: 'range-2', kind: 'generic', sourceStart: 2, sourceEnd: 4 }]) satisfies TerminalRangeIndex",
      ...
      'getTerminalSearchSessionStats(terminalSearch) satisfies TerminalSearchSessionStats',
      'getTerminalSearchSessionStats(terminalSearch).matchLimit satisfies number | null',
      'getTerminalSearchSessionStats(terminalSearch).truncated satisfies boolean',
      "const terminalSearchCappedOptions: TerminalSearchOptions = { matchLimit: 1 }",
      'createTerminalSearchSession(prepared, terminalSearchQuery, terminalSearchCappedOptions) satisfies TerminalSearchSession',
