...
Current priorities for maintaining the `0.2.1` `pretext-TUI` host-neutral terminal text kernel.
...
- `letterSpacing` is no longer accepted by internal `PrepareOptions`, but fixed-zero geometry fields and dead line-break branches remain as migration residue — prune those in a behavior-frozen cleanup if future debug-snapshot diffs stay byte-identical.
...
- WASM/icu4x analysis kernel: REJECTED AND CLOSED (measured) — icu4x is a third segmentation behavior that cannot be probe-pinned byte-identical to the live engine `Intl.Segmenter` over its non-ASCII value-add region, the tarball story is zero-runtime-dep, and the wasm toolchain is absent from CI. A 2026-06-11 adversarial re-examination built the strongest constraint-legal kernel (pinned ASCII subset only, DFA parameters as probe-extracted data) and measured its free-function ceiling at about 5% of warm prepare with a realistic net under 3% after the JS allocation floor, while the non-ASCII value region stays unpinnable by construction — re-open condition (1) is numerically foreclosed for transcript-shaped corpora. The three re-open conditions stand unchanged otherwise.
- Lazy newline-bounded chunk-deferred prepare: the designated successor to the closed WASM question — dissolves cold full-prepare latency instead of accelerating the analyzer. Open as its OWN future incubating track with an approval record per docs/decisions/incubating-api-approval-index.md; v1 pre-wrap only; pinned by a one-shot whole-vs-chunked parity gate over the A/B corpus net (measured: chunked analysis carries no penalty, byte-identical parity over 1.5M segments, honest cold time-to-tail about 6x). The shipped cell-flow append path now avoids the former reader-store O(totalSize) rebuild through chunked reader-store append; keep long-unbroken-tail behavior and normal-mode seal-safety under the separate append follow-ups below.
- Cell-flow normal-mode seal-safety audit: `findTerminalCellFlowSealOffset` seals chunks at spaces, but live adjacency probes show many hostile space junctions are not segmentation-safe boundaries; add an independent parity audit test over hostile junction corpora for the shipped sealing behavior.
- Width-profile engine-float fix: the four Unicode-property regexes in src/unicode/terminal-string-width.ts make `terminal-unicode-narrow@1` float with the host engine's Unicode data (hundreds of code points measured with different widths between bun and node at the same profile version). Freeze them into a generated range table swept on the pinned gate runtime, following the scripts/generate-bidi-data.ts precedent, with an exhaustive all-code-points differential at landing; the emojiWidth A/B axis gap joins this series.
- Rule-predictable UAX-29 scanner extension (probe-pin pattern beyond ASCII): experiment-complete and byte-identical on both engines across 12 locales and 20k fuzz trials, removing about a third of prepare time on Korean-heavy content — but the headline mixed benchmark moves zero, so it lands only with recorded host demand for ko/ar/he/hi transcript workloads AND a gated ko benchmark workload added first, never alone.
- Host-injected parallel span-batch analysis: measured around 2x end-to-end on pre-wrap corpora with byte-identical output across worker counts, but it needs a new incubating async entry and the host-injected capability pattern; sequenced behind the injected width-profile landing, the measurement levers above, and the lazy-prepare track decision (if that track lands, re-derive on its chunk store instead of a separate batch planner).
...
- R4 adoption surface: the terminal conformance kit under fixtures/conformance/ is repo-only data verified by `bun run conformance-kit-check` and `bun run scripts/tui-conformance-kit-generate.ts --check` inside `release-gate:tui`; keep it out of the npm tarball unless a separate publish-surface decision lands.
...
- Cite report ids such as `competitive-tui-20260615-05a8d54-clean-ad380eea` only with workload ids and semantic caveats; do not copy dynamic timing numbers into prose.

## Completed

- Width-profile hardening has landed: resolved profiles are module-branded, injected profiles are branded by their factory, structural copies with stale `cacheKey` are normalized, and the unused cache-key helper was removed.
- `mergeUrlLikeRuns` no longer performs the no-op `kinds[j] = 'text'` write inside its URL merge loop.
- The `0.2.1` cell-flow append fix landed the chunked reader-store path, keeping 1,000-small-append benchmark evidence on the O(window) analyzed-source path instead of rebuilding the full reader store per append.
...
- Track R4 in-repo adoption surface has landed: the core seven are promoted to stable 0.1 with an exact partition assertion, a repo-only terminal conformance fixture kit is generated, engine-checked, release-gated, and still excluded from the npm tarball, and the Unicode upgrade policy governs future width-profile versions.
